...
export function AwardEntryDialog({ open, initialValues, userId, onClose, onSave }: AwardDialogProps) {
  const t = useTranslations("profile.dialogs.award");
    ...
    <DialogShell title={initialValues ? t("editTitle") : t("addTitle")} onClose={onClose}>
      ...
      >
        ...
        <div className="grid gap-5 md:grid-cols-2">
          <FormField label={t("issuedBy")} error={errors.issuedBy?.message}>
            <input {...register("issuedBy")} className="ds-input" placeholder={t("issuedByPlaceholder")} />
          </FormField>
          <FormField label={t("date")} error={errors.date?.message}>
            <DatePickerField
        ...
        <FormField label={t("referenceLink")} error={errors.referenceUrl?.message} hint={t("optional")}>
          <input {...register("referenceUrl")} className="ds-input" placeholder={t("referenceLinkPlaceholder")} />
        </FormField>
